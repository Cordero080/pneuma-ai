...
// detectUnanswerable — matches questions that have no knowable answer (meaning of life, death, god). No deps.
...
// detectParadox — matches self-referential or logically contradictory questions. No deps.
...
// detectPersonalToUser — matches questions only the user can answer about their own life or choices. No deps.
...
// detectFuturePrediction — matches requests to forecast outcomes or predict what will happen. No deps.
...
// detectExistential — matches deep meaning/existence questions, boosted by philosophical intent scores. No deps.
...
// detectCertaintySeek — detects when a user is explicitly demanding certainty or a definitive answer. No deps.
...
// calculateUncertaintyScore — aggregates detector flags into a 0–1 uncertainty score. No deps.
 ...
 */
// [1] analyzeUncertainty — scores the message for uncertainty signals. Calls private detectors. No deps.
...
// containsQuestion — returns true if the message contains a direct or embedded question. No deps.
...
// isProcessing — returns true if the message is a short emotional utterance with no real content. No deps.
...
// [2] shouldBeQuiet — decides if Pneuma should say less. Waits for: [1].
 ...
 */
// [3] detectPerspectiveShift — detects if Pneuma changed its position. No deps (standalone check).
...
// [4] getUncertaintyResponse — builds uncertainty-aware response. Waits for: [1].
...
// [5] getQuietResponse — returns a brief quiet response. Waits for: [2].
 ...
 */
// [6] getMindChangePhrase — returns a phrase acknowledging a changed view. Waits for: [3].
 ...
 */
// [7] getFigureItOutResponse — returns a collaborative "let's think" phrase. No deps.
