export const TERM_REGISTRY = {
  ...
  "knowledge pipeline": {
    ...
    inPneuma:
      "Stage 1: raw primary texts pasted into docs/interview/text-prompt-ref/ as source material. Stage 2: scripts/ingest_passages.py machine-chunks each source by size (120-1,200 chars), then Claude Sonnet selects the ~35 most distinctive passages verbatim, written into data/archetype_knowledge/{thinker}/passages.json with themes + context annotations (machine-chunked, LLM-curated — not hand-extracted). Stage 3: archetypeRAG.js embeds all passages once at startup and caches them in archetype_embeddings.json (51MB). New passages in any passages.json are auto-embedded on next server start.",
  },
  "cosine similarity": {
    ...
    inPneuma:
      "Every search in Pneuma runs on this. archetypeRAG.js keeps passages scoring above 0.3. vectorMemory.js keeps memories above 0.35. archetypeSelector.js ignores any archetype below 0.7. Higher threshold = stricter match.",
  },
  vector: {
  ...
  CONTRAST_MAP: {
    ...
    inPneuma:
      "CONTRAST_MAP is now a secondary tiebreaker in the Concept Crossroads evaluation pipeline. The primary contrast mechanism is the collision bonus in _evaluatePassages(): passages from thinkers in known tension get a flat 0.2 score bonus (added, not a 20% weight), regardless of which thinker scored highest. CONTRAST_MAP only activates after that scoring pass, as a final diversity check.",
  },
  "dual RAG": {
