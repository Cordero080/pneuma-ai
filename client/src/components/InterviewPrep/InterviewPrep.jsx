...
const FILE_REGISTRY = {
  ...
  "archetypeSelector.js": {
    ...
    whatItDoes:
      "Embeds the user's message. Compares it via cosine similarity against pre-computed embeddings of each archetype's essence description. Returns the closest match only if it clears the 0.7 bar, else null. buildArchetypeContext() passes that 0.7 threshold in as an argument, so there's a single gate now, not a 0.25 floor plus a separate 0.7 check.",
    flowChain:
...
For philosophical messages, the Concept Crossroads path runs: extractConcepts() detects which of ~80 tracked philosophical concepts are present in the message — time, death, suffering, consciousness, freedom, love, paradox, and others. For each detected concept, it fires a parallel embedding query formatted as "{concept} {thinker}" for each active thinker via Promise.all(). _evaluatePassages() then scores every result as: relevance to the message × 0.5 + distinctiveness from the other pool candidates × 0.3 + a flat 0.2 collision bonus (added, not a 20% weight) if the thinker pair is known to disagree. It is not a 50/30/20 split. _selectBestPassages() deduplicates near-identical results (cosine > 0.95), caps at two passages per thinker, and fills to topK=8. This re-rank runs only on the multi-query path; the single-query fallback ranks by raw cosine alone.
