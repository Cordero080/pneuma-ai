...
export const UserInputModal = ({ isOpen, onClose, anchorEl }) => (
  ...
  >
    <ModalSection title="Message Entry Point">
      <ModalDesc>
        When you type a message in the Pneuma chat interface, it travels from
        your browser to the backend server through an HTTP POST request.
      </ModalDesc>
  ...
  // Stream response back to UI...
    ...
    <ModalSection title="Request Flow">
      <ModalFlow
        steps={[
          {
            title: "Browser → Vite Dev Server",
            desc: "React app sends POST to /api/message",
          },
          {
            title: "Proxy to Express",
            desc: "Vite proxies to localhost:3001 (server)",
          },
          {
            title: "Express Route Handler",
            desc: "server/index.js receives the request",
          },
          {
            title: "pneumaRespond() Called",
            desc: "Orchestration begins in core/fusion.js",
          },
        ]}
      />
    </ModalSection>
...
export const EntryPointModal = ({ isOpen, onClose, anchorEl }) => (
  ...
  >
    <ModalSection title="Express Route Handler">
      <ModalDesc>
        The Express server receives the POST request and immediately invokes the
        main orchestration function. This is the gateway between HTTP and
        Pneuma's cognitive pipeline.
      </ModalDesc>
  ...
  try {
    ...
    <ModalSection title="Request Journey">
      <ModalFlow
        steps={[
          {
            title: "Receive HTTP Request",
            desc: "Express parses JSON body from req.body",
          },
          {
            title: "Set Up Streaming",
            desc: "Configure Server-Sent Events (SSE) for real-time response",
          },
          {
            title: "Invoke Orchestrator",
            desc: "Call pneumaRespond() from core/fusion.js",
          },
          {
            title: "Stream Chunks",
            desc: "As LLM generates, chunks are streamed back to client",
          },
    ...
    <ModalSection title="Streaming Architecture">
      <ModalDesc>
        Pneuma uses Server-Sent Events (SSE) to stream the response
        token-by-token. This creates the "typing" effect you see in the chat
        interface.
      </ModalDesc>
    ...
    <>
      ...
      >
        <ModalSection title="The Conductor">
          <ModalDesc>
            The orchestrator is the central coordinator that manages the entire
            response generation pipeline. It loads state, checks special modes,
            retrieves memories, and coordinates all subsystems.
          </ModalDesc>
        ...
        <ModalSection title="Orchestration Flow">
          <ModalFlow
            steps={[
              {
                title: "Load Pneuma State",
                desc: "Read current emotional state, active archetypes, conversation context from pneuma_state.json",
              },
              {
                title: "Check Special Modes",
                desc: "Detect dream mode, reflection mode, or special invocation states",
              },
              {
                title: "Load Long-Term Memory",
                desc: "Query vector memory for relevant past conversations",
              },
              {
                title: "Build Context",
                desc: "Assemble conversation history + memory + state into coherent context",
              },
              {
                title: "Invoke Response Engine",
                desc: "Pass to responseEngine.js for intent/tone detection",
              },
            ]}
          />
        </ModalSection>
...
async function loadState() {
  ...
  return {
    ...
    lastInteractionTime: state.lastInteractionTime
        ...
        <ModalSection title="Memory Retrieval (Hybrid)">
          <ModalDesc>
            Before generating a response, Pneuma builds a memory context from
            two sources. The last 4 conversation turns are always included first
            — guaranteed recency. Then semantically similar older exchanges from{" "}
            <ModalLink onClick={() => setNestedModal("rag")}>
              vector search
            </ModalLink>{" "}
            are appended after, deduplicated so nothing appears twice.
          </ModalDesc>
          <ModalFilePath path="server/pneuma/memory/vectorMemory.js" />
          <ModalCodeBlock>{`// llm.js — hybrid memory assembly
const recentTurns = getCurrentExchanges(4);   // always included
const semantic = await retrieveMemories(msg); // score > 0.35

// Deduplicate: drop semantic results that overlap recent turns
// Inject as two labeled blocks in the system prompt:
// "RECENT CONVERSATION (LAST 4 TURNS)" → then →
// "SEMANTICALLY RELEVANT OLDER MEMORIES"`}</ModalCodeBlock>
        </ModalSection>
      </Modal>
      ...
      >
        <ModalSection>
          <ModalDesc>
            Dream Mode activates when there's been an extended silence (30+
            minutes) or when Pneuma detects contemplative/nocturnal themes.
          </ModalDesc>
  ...
  return silenceDuration > 30 * 60 * 1000 || 
      ...
      >
        <ModalSection title="What is RAG?">
          <ModalDesc>
            RAG combines the power of a language model with a searchable
            knowledge base. Instead of relying only on training data, Pneuma
            retrieves relevant context from its own memories and archetype
            knowledge before generating.
          </ModalDesc>
        </ModalSection>
        <ModalSection title="How Pneuma Uses RAG">
          <ModalFlow
            steps={[
              {
                title: "Embed User Message",
                desc: "Convert message to vector using embedding model",
              },
              {
                title: "Search Vector Store",
                desc: "Find similar vectors in memory database",
              },
              {
                title: "Retrieve Context",
                desc: "Pull actual text content of top matches",
              },
              {
                title: "Inject into Prompt",
                desc: "Add retrieved context to system prompt",
              },
            ]}
          />
        </ModalSection>
        <ModalExample label="RAG in Action">
          User asks: "What did we discuss about my father?"
          <br />
          <br />
          RAG retrieves: "3 weeks ago, user explored relationship with father
          figure, themes of approval-seeking and shadow projection..."
        </ModalExample>
      </Modal>
      ...
      >
        <ModalSection>
          <ModalDesc>
            Reflection Mode engages deeper archetype layers and more
            contemplative response patterns. Triggered by explicit requests like
            "reflect on..." or "what do you think about your own..."
          </ModalDesc>
        </ModalSection>
      </Modal>
      ...
      >
        <ModalSection>
          <ModalDesc>
            Shadow Mode activates when detecting conflict, defensiveness, or
            topics the user may be avoiding. Injects archetypes like Nietzsche,
            Dostoevsky, or the Jungian shadow to provide honest challenge rather
...
export const IntentDetectionModal = ({ isOpen, onClose, anchorEl }) => {
  ...
  const intents = [
    {
      name: "CASUAL",
      desc: "Light conversation, greetings",
      example: '"Hey, how are you?"',
      weight: "0.0-1.0",
    },
    {
      name: "EMOTIONAL",
      desc: "Feelings, personal struggles",
      example: '"I\'ve been feeling lost lately"',
      weight: "0.0-1.0",
    },
    {
      name: "PHILOSOPHICAL",
      desc: "Deep questions, meaning",
      example: '"What is consciousness?"',
      weight: "0.0-1.0",
    },
    {
      name: "NUMINOUS",
      desc: "Spiritual, transcendent",
      example: '"I had a mystical experience"',
      weight: "0.0-1.0",
    },
    {
      name: "CONFLICT",
      desc: "Disagreement, challenge",
      example: '"I think you\'re wrong about..."',
      weight: "0.0-1.0",
    },
    {
      name: "INTIMACY",
      desc: "Connection-seeking, vulnerable",
      example: '"Can I tell you something personal?"',
      weight: "0.0-1.0",
    },
    {
      name: "HUMOR",
      desc: "Playful, joking",
      example: '"Tell me a joke about philosophers"',
      weight: "0.0-1.0",
    },
    {
      name: "CONFUSION",
      desc: "Uncertainty, seeking clarity",
    ...
    <>
      ...
      >
        <ModalSection title="Multi-Intent Classification">
          <ModalDesc>
            Rather than picking a single intent, Pneuma scores the user's
            message across 8 intent dimensions simultaneously. Each gets a
            weight from 0.0 to 1.0, allowing for nuanced understanding of
            complex messages.
          </ModalDesc>
      ...
      {intents.map((intent) => (
        ...
        >
          ...
          <ModalSection title="Detection Triggers">
            <ModalExample label="Typical Message">
              {intent.example}
            </ModalExample>
          </ModalSection>
          <ModalSection title="Response Implications">
            <ModalDesc>
              When {intent.name.toLowerCase()} intent is high, Pneuma adjusts
              tone and archetype selection to match. Higher{" "}
              {intent.name.toLowerCase()} scores pull toward appropriate
              response styles.
            </ModalDesc>
          </ModalSection>
        </Modal>
  ...
  return weightedRandom(weights);
        ...
        <ModalSection title="Intent → Tone Mapping">
          <ModalExample label="Example">
            High emotional + high intimacy intent → likely INTIMATE tone
            <br />
            High philosophical + high numinous → likely ORACULAR tone
            <br />
...
export const ArchetypeSelectionModal = ({ isOpen, onClose, anchorEl }) => {
    ...
    <>
      ...
      >
        <ModalSection title="6-Way Selection Process">
          <ModalDesc>
            This is where Pneuma's unique personality emerges. Six different
            methods work together to select 3-4 archetypes that will shape the
            response. The combination creates emergent voice.
          </ModalDesc>
  ...
  return [...selected].slice(0, 4);
        ...
        <ModalSection title="Cognitive Methods Architecture (Jan 2026)">
          <ModalDesc>
            Beyond selection, archetypes now carry{" "}
            <strong>cognitive operations</strong> — actual thinking moves that
            get injected as available tools. This is "cognitive metabolization"
            vs. just quoting.
          </ModalDesc>
          <ModalCodeBlock>{`// ARCHETYPE_METHODS in llm.js
{
  ...
  // ... 5 archetypes have cognitive methods so far
          ...
          <ModalExample label="How it's used">
            System prompt includes: "You can use [anatomize] to dissect the
            question, [sfumato_edges] to blur categories..."
            <br />
            <br />
  ...
  STRATEGIC: ['suntzu', 'musashi', 'aurelius']
      ...
      >
        <ModalSection title="What is Vector Matching?">
          <ModalDesc>
            Every archetype has their core ideas converted into mathematical
            vectors (embeddings). When you send a message, it's also converted
            to a vector. We then find which archetype vectors are most similar
            to your message vector.
          </ModalDesc>
        </ModalSection>
        ...
        <ModalSection title="How It Works">
          <ModalFlow
            steps={[
              {
                title: "Embed Message",
                desc: "Convert 'What is consciousness?' to a 1536-dimension vector",
              },
              {
                title: "Compare to Archetypes",
                desc: "Calculate cosine similarity with each archetype's knowledge vectors",
              },
              {
                title: "Rank Results",
                desc: "Sort by similarity score (0.0 to 1.0)",
              },
              {
                title: "Select Top Matches",
                desc: "Pick top 2 archetypes above threshold (0.65)",
              },
            ]}
          />
        </ModalSection>
  ...
  return scores
    ...
    .slice(0, topK);
        ...
        <ModalExample label="Example">
          Message: "I feel like I'm not living authentically"
          <br />
          <br />
